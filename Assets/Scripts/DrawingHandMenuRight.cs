...
public class DrawingHandMenuRight : MonoBehaviour
{
    ...
    [SerializeField] private RightStartColorPicker  colorPickerStart;
    ...
    [SerializeField] private RightEndColorPicker    colorPickerEnd;
    ...
    void Start()
    {
        ...
        {
            Debug.Log($"DrawingHandMenuRight: Configured right menu key = {DesktopInputController.Instance.GetMenuKeyDisplayName(false)}");
        }
        else
        {
            Debug.Log("DrawingHandMenuRight: DesktopInputController.Instance is null at Start");
        }
    ...
    private void OnRightMenuPressed_Event()
    {
        handMenuEnableState = !handMenuEnableState;
        SetPanelActiveStates();
        if (handMenuEnableState)
        {
            MoveMenuToCameraView(isLeft: false);
            if (Crosshair.Instance != null) Crosshair.Instance.ShowToast("Right menu opened");
        }
    }
    ...
    private void HandleDesktopToggle()
    {
        ...
        UnityEngine.InputSystem.Key keyToCheck = UnityEngine.InputSystem.Key.F2;
        if (DesktopInputController.Instance != null)
        {
            keyToCheck = DesktopInputController.Instance.RightMenuKey;
        }

        var keyControl = Keyboard.current[keyToCheck];
        if (keyControl != null && keyControl.wasPressedThisFrame)
        {
            Debug.Log($"DrawingHandMenuRight: Detected key press for {keyToCheck}");
            handMenuEnableState = !handMenuEnableState;
            SetPanelActiveStates();
            if (handMenuEnableState)
            {
    ...
    private void OnDestroy()
    {
        ...
        {
            DesktopInputController.Instance.RightMenuPressed -= OnRightMenuPressed_Event;
    ...
    private void SubscribeToController()
    {
        ...
        if (DesktopInputController.Instance != null)
        {
            ...
            Debug.Log("DrawingHandMenuRight: Subscribed to DesktopInputController.RightMenuPressed");
        }
    }
